...
export default function Micronutrients({
  ...
  // Oduzimamo visinu naslova i margina da lista stane tačno unutar izmerenog prostora
  const listMaxHeight = maxHeight ? `${maxHeight - 40}px` : "360px";

  return (
    <section className="flex w-full min-w-0 flex-col">
      <div className="flex shrink-0 items-center justify-between mb-3 sm:mb-5">
        <h2 className="text-lg sm:text-xl font-bold text-text">Micronutrients</h2>
      </div>

      <div
        style={{ maxHeight: listMaxHeight }}
        className="custom-scrollbar w-full min-w-0 space-y-2 overflow-y-auto pr-1 sm:space-y-2.5 sm:pr-2"
      >
        {micros.map((micro) => {
          ...
          return (
            <article
              key={micro.name}
              className="card-main w-full min-w-0 px-3 py-2.5 sm:px-4 sm:py-3"
            >
